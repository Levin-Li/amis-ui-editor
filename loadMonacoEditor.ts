...
// 所以会被 fis3 替换成取相对的代码。
function filterUrl(url: string, filter: (info: string) => string = (info => info)) {
    return filter(url);
}

function onLoad(req: any, callback: (result: any) => void) {

    const basePath = __uri('monaco-editor/min/vs/editor/editor.main.js').replace(
        /\/vs\/.*$/,
        ''
    );
    ...
    req([config.paths.vs + '/vs/editor/editor.main'], function (ret: any) {
        callback(ret);
    });
}
